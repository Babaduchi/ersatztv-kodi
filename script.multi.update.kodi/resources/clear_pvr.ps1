param(
    ...
    [Parameter(Mandatory = $true)][string]$KodiExecutable,
    [Parameter(Mandatory = $true)][int]$KodiProcessId
)
...
try {
    ...
    while ((Get-Process -Id $KodiProcessId -ErrorAction SilentlyContinue) -and ((Get-Date) -lt $deadline)) {
